...
# ── P6-2: 위험군별 tier 하한 (2026-07-05 전면 개방) ──
# 구 정책은 "risk_0001=취업 안정권"이라는 5단계 체계 가정으로 risk_0001에 산업기사 이상만,
# risk_0002에 기능사 상위 이상만 허용했다. 4단계 전환(risk_0001=고립위험청년, 가장 경미) 이후
# 이 가정은 성립하지 않는다 — LLM-judge recall 평가에서 risk_0001·0002 사용자에게 기능사 등
# 접근 가능한 자격증이 원천 배제되는 회귀로 발견됨(DEV_LOG 2026-07-05).
# 이제 tier 기반 배제는 candidate 생성 단계(recommended_risk_stages, relations/FOLDER.md §P6-1)의
# "기술사·기능장은 활동제한형·은둔청년 제외"만으로 충분하므로, 여기서는 전부 0(비활성)으로 둔다.
# 같은 위험군 안의 난이도 우선순위는 _fit_score의 difficulty_fit이 소프트하게 처리한다.
