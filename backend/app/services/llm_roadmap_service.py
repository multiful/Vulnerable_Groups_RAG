...

{
  ...
  "s3": "문장3: '지금 상황에서는' 또는 '현재 상황을 고려하면' 으로 시작 + 연 N회 시험 기준 M개월 준비 계획 + 취득 후 구체적 결과"
}
...
s3: 반드시 "지금 상황에서는" 또는 "현재 상황을 고려하면" 으로 시작
    / "위험군", "고립청년", "은둔청년" 등 분류 명칭을 절대 사용하지 말 것
    / 반드시 "N개월" 또는 "상·하반기" 포함
...
• "위험군", "고립청년", "은둔청년", "활동제한형", "활동형고립" ← s3에서 절대 금지
...
"""
    ...
    _FS_ASST_FULL = (
        ...
        '"s3":"지금 상황에서는 연 2회 시험 주기를 기준으로 '
        "6개월 집중 준비 루틴을 구성하면 첫 성과가 구체적으로 확정됩니다.\"}"
    )
    _FS_USER_LIMITED = (
    ...
    _FS_ASST_LIMITED = (
        ...
        '"s3":"현재 상황을 고려하면 6~8개월 집중 학습으로 연 2회 시험 중 '
