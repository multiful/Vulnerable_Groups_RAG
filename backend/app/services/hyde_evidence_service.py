...
def _synthesize_with_guardrail(
    ...
    system_prompt = (
        ...
        "6. 첫 문장에 반드시 [분류 결과]의 주요 어려움 차원명과 수치를 인용하세요.\n"
        f"   예: '{dim_summary.split('/')[0].strip() if dim_summary else '관계망(XX%)'}에서의 높은 어려움으로 인해 ...'"
    )

    user_prompt = (
        f"[분류 결과]\n"
        f"유형: {cluster_name}\n"
        f"주요 어려움 차원(반드시 첫 문장에 인용): {dim_summary}\n\n"
        f"[제공 문서]\n{doc_texts}\n\n"
        f"위 제공 문서를 근거로, 이 사용자가 '{cluster_name}'으로 분류된 이유를 "
        f"사용자가 납득할 수 있도록 설명하세요.\n"
        f"첫 문장에 '{dim_summary}' 수치를 반드시 언급하고 각 문장에 [문서 N] 인용 필수."
    )
